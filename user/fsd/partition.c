...
void partition_init()
{
    ...
    for (uint64_t i = 0; i < block_dev_num; i++)
    {
        ...
        lseek(blkdev_fd, 512);
        read(blkdev_fd, buffer, sizeof(struct GPT_DPT));
        ...
        lseek(blkdev_fd, buffer->partition_entry_lba * 512);
        read(blkdev_fd, dptes, buffer->num_partition_entries);
    ...
    probe_mbr:
        ...
        lseek(blkdev_fd, 0x8001);
        read(blkdev_fd, iso9660_detect, 5);
        ...
        lseek(blkdev_fd, 0);
        read(blkdev_fd, boot_sector, sizeof(struct MBR_DPT));
