...
uint64_t get_mmio_address(uint32_t pci_address, uint16_t offset)
{
    ...
    uint64_t virt = physmap(phys, 0x1000, PROT_READ | PROT_WRITE);
...
uint32_t pci_read(uint32_t b, uint32_t d, uint32_t f, uint32_t s, uint32_t offset)
{
    ...
    if (mmio_address == 0)
    {
        printf("Cannot read pci: failed to get mmio address");
    }
...
void pci_write(uint32_t b, uint32_t d, uint32_t f, uint32_t s, uint32_t offset, uint32_t value)
{
    ...
    if (mmio_address == 0)
    {
        printf("Cannot write pci: failed to get mmio address");
    }
...
void pci_scan_device_legacy(uint32_t bus, uint32_t equipment, uint32_t f)
{
    ...
    printf("Found PCI device: %#08lx name: %s\n", device->class_code, device->name);
    ...
    for (int i = 0; i < 6; i++)
    {
        ...
        else
        {
            ...
            else if (bar_type == 0x2)
            {
                if (i >= 5)
                {
                    printf("Error: 64-bit BAR at position overflow\n");
                    continue;
                }
...
void init_pci(MCFG *mcfg_buffer, bool pcie)
{
    if (pcie)
    {
