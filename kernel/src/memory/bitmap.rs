...
impl Bitmap {
    ...
    pub fn find_range_from(&mut self, length: usize, value: bool, start: usize) -> Option<usize> {
        let byte_match = if value { usize::MAX } else { 0 };
        let (mut count, mut start_index) = (0, start);
    ...
    pub fn find_range(&mut self, length: usize, value: bool, start: usize) -> Option<usize> {
        self.find_range_from(length, value, 0)
    }
}
