...
use rmm::{Arch, BumpAllocator, FrameAllocator, FrameCount, FrameUsage, PhysicalAddress};
use spin::Mutex;
...
const _: () = {
    assert!(core::mem::size_of::<PageInfo>().is_power_of_two());
};
...
fn init_sections(mut allocator: BumpAllocator<RmmA>) {
    ...
    let sections: &'static mut [Section] = {
        ...
        let section_array_page_count =
            (max_section_count * core::mem::size_of::<Section>()).div_ceil(PAGE_SIZE);
    ...
    while let Some(mut memory_map_area) = iter.next() {
        ...
        while pages_left > 0 {
            ...
            let page_info_array_size_pages =
                (page_info_count * core::mem::size_of::<PageInfo>()).div_ceil(PAGE_SIZE);
...
pub fn init_mm(allocator: BumpAllocator<RmmA>) {
    ...
    log::info!("Initial memory consistent");
}
