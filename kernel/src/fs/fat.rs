use crate::{ref_to_mut, ref_to_static, unsafe_trait_impl};
...
use super::vfs::{
    get_inode_by_path,
    inode::{FileInfo, Inode, InodeRef, InodeTy},
};
...
impl Inode for Fat32Volume {
    ...
    fn open(&self, name: String) -> Result<InodeRef, SystemError> {
        ...
        let self_inode = get_inode_by_path(self.get_path()).unwrap();
        ...
        if let Some(inode) = self.virtual_inodes.get(&name) {
            ...
            inode
                .write()
                .when_mounted(self.get_path() + name.as_str() + "/", Some(self_inode));
            return Ok(inode);
    ...
    fn create(
        &self,
        name: String,
        ty: super::vfs::inode::InodeTy,
    ) -> Result<InodeRef, SystemError> {
        match ty {
            InodeTy::Dir => {
                self.vol
                    .root_dir()
                    .create_dir(name.as_str())
                    .or(Err(SystemError::EEXIST))?;
            }
            InodeTy::File => {
                self.vol
                    .root_dir()
...
impl Inode for Fat32File {
    ...
    fn read_at(&self, offset: usize, buf: &mut [u8]) -> Result<usize, SystemError> {
        let mut size = 0;
    ...
    fn write_at(&self, offset: usize, buf: &[u8]) -> Result<usize, SystemError> {
        let mut size = 0;
        ref_to_mut(self.file.as_ref())
            .seek(SeekFrom::Start(offset as u64))
            .unwrap();
