...
use crate::{
    errno::Errno,
    syscall::{slice_from_user, slice_from_user_mut},
};
...
impl IoVecs {
    ...
    #[inline(never)]
    pub unsafe fn from_user(iov: *const IoVec, iovcnt: usize, _readv: bool) -> Result<Self, Errno> {
        let iovs_reader = slice_from_user(iov, iovcnt).ok_or(Errno::EFAULT)?;

        let mut slices: Vec<IoVec> = Vec::with_capacity(iovs_reader.len());

        for iov in iovs_reader.iter() {
            if iov.iov_len == 0 {
                continue;
            }

            let _ = slice_from_user(iov.iov_base, iov.iov_len).ok_or(Errno::EFAULT)?;
            slices.push(*iov);
        }
    ...
    pub fn gather(&self) -> Vec<u8> {
        ...
        for slice in self.0.iter() {
            let buf_reader = slice_from_user(slice.iov_base, slice.iov_len).unwrap();
            buf.extend_from_slice(buf_reader);
        }
        return buf;
    }
    ...
    pub fn scatter(&self, data: &[u8]) {
        ...
        for slice in self.0.iter() {
            ...
            let buf_writer = slice_from_user_mut(slice.iov_base, slice.iov_len).unwrap();

            buf_writer[..len].copy_from_slice(&data[..len]);
            data = &data[len..];
        }
    }
