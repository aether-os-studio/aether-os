...
use crate::fs::{
    vfs::get_root_partition_uuid,
    vfs::{
        dev::{ROOT_PARTITION, partition::PartitionInode},
...
impl BlockIo for InodeRefIO {
    ...
    fn read_blocks(
        ...
        if let Ok(_bytes) = self.inode.read().read_at(start_lba.0 as usize * 512, dst) {
            return Ok(());
        }
        Err(1)
    }
    ...
    fn write_blocks(
        ...
        if let Ok(_bytes) = self.inode.read().write_at(start_lba.0 as usize * 512, src) {
            return Ok(());
        }
        Err(1)
    }
