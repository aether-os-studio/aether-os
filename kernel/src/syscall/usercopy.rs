use crate::{
    arch::{CurrentSyscallArch, syscall::SyscallArch},
    syscall::error::{EFAULT, EINVAL, Error, Result},
};
...
impl<const WRITE: bool> UserSlice<true, WRITE> {
    pub fn copy_to_slice(self, slice: &mut [u8]) -> Result<()> {
        ...
        if unsafe {
            CurrentSyscallArch::copy_from_user(slice.as_mut_ptr() as usize, self.base, self.len)
        } == 0
        {
            Ok(())
        } else {
...
impl<const READ: bool> UserSlice<READ, true> {
    pub fn copy_from_slice(self, slice: &[u8]) -> Result<()> {
        ...
        if unsafe { CurrentSyscallArch::copy_to_user(self.base, slice.as_ptr() as usize, self.len) }
            == 0
        {
            Ok(())
        } else {
